        ...
        <div className="max-w-5xl mx-auto space-y-4 md:space-y-6">
          {sortedTeams.length === 0 ? (
            ...
            sortedTeams.map((team, index) => {
              const isLowHealth = team.health > 1 && team.health <= 4;
              const isCriticalHealth = team.health === 1;
                ...
                <div key={team.id} className="w-full">
                  ...
                  >
                    <div className="flex items-center gap-4 md:gap-6 mb-4 md:mb-0 w-full md:w-1/3">
                      <span
                        className="font-orbitron text-2xl md:text-3xl font-black opacity-40 transition-opacity group-hover:opacity-100 min-w-[2rem]"
                    ...
                    <div className="flex items-center gap-1 md:gap-1.5 justify-center flex-1 mb-4 md:mb-0">
                      {[...Array(10)].map((_, i) => {
                        ...
                        const bounceClass = isFilled
                          ? isCriticalHealth
                            ? "minecraft-heart-critical-bounce"
                            : isLowHealth
                              ? "minecraft-heart-bounce"
                              : ""
                          : "";
                    ...
                    <div
                      className="absolute top-0 right-0 w-3 h-3 md:w-4 md:h-4 border-t-2 border-r-2"
      ...
      <div className="bg-[#050518]/80 backdrop-blur-md border-t border-white/5 p-3 md:p-4 relative z-10 overflow-hidden">
        <div className="flex items-center gap-8 md:gap-16 animate-marquee whitespace-nowrap">
          {[1, 2, 3].map((i) => (
            <span
              key={i}
              className="text-gray-500 font-mono text-[8px] md:text-[10px] uppercase tracking-[0.3em] md:tracking-[0.5em]"
            >
              * SYSTEM ALERT: <span className="text-yellow-500">BOUNTY PROTOCOL ACTIVE (50%)</span> * 0 HP RESULTS IN
              IMMEDIATE VOID * ALL ABILITIES ARE ONE-TIME USE * TRACK YOUR VITALS *
            </span>
          ))}
        </div>
      </div>
